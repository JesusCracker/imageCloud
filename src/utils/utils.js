...
export function getLastTime(targetTime) {
  let result = '';
  // 今天开始时间
...
// 是否是今天
  if(moment(targetDate).isSame(today, 'day')) {
    result = `今天 ${ moment(targetTime).format('HH:mm')}`;
  } else if(moment(targetDate).isSame(yesterday, 'day')) {
    // 是否是昨天
    result = `昨天 ${ moment(targetTime).format('HH:mm')}`;
  } else if(moment(targetDate).isSame(today, 'year')) {
    // 是否是今年
    result = moment(targetTime).format('MM-DD HH:mm:ss');
  } else if(targetTime){
